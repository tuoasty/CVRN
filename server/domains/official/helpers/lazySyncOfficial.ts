...
export async function lazySyncOfficial(
    ...
    try {
        ...
        const updateResult = await updateOfficial(supabase, {
        ...
        if (!updateResult.ok) {
            logger.error({robloxUserId: String(user.id), error: updateResult.error}, "Failed to update official during lazy sync");
            return updateResult;
        }
